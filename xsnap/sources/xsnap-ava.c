...
xsBooleanValue xsMeteringCallback(xsMachine* the, xsUnsignedValue index)
{
...
static void xs_issueCommand(xsMachine *the)
{
	...
	int writeError = fxWriteNetString(toParent, "?", buf, len);
	if (writeError != 0) {
